...
(function() {
    ...
    function handleClick(x, y) {
        // SIMPLIFIED: Click ANY active object under cursor
        // Find the first object under the click (simple and reliable)
        for (let obj of objectPool) {
            if (obj.active && obj.contains(x, y)) {
                console.log(`Clicked ${obj.type} at (${x.toFixed(0)}, ${y.toFixed(0)})`);
                ...
                checkAchievements();
                return; // Exit after first click
            }
        }
        
        console.log(`No object found at (${x.toFixed(0)}, ${y.toFixed(0)})`);
    }
    ...
    function showAgeConsentModal() {
        ...
        if (!modal) {
            console.error('❌ Age consent modal not found!');
            return;
        }
